...
| Architectural Concern | Existing Prototype (`frontend/src/`) | Target Architecture State | Implementation Status (Phase P09) |
|---|---|---|---|
| **Routing & Shell Layout** | `AppShell.tsx` hosts `react-router-dom` (`/orders`, `/settlement`, `/analytics`, `/catalog`). | `AppShell` hosts `Routes` via `react-router-dom` with active tab highlighting. | **COMPLETE & ACTIVE:** Standard React Router DOM routes integrated in `App.tsx`. |
| **Page Layer Separation** | Pages organized cleanly under feature directories. | Dedicated orchestrators (`OrdersPage.tsx`, `SettlementsPage.tsx`, `AnalyticsDashboardPage.tsx`, `CatalogPage.tsx`). | **COMPLETE & ACTIVE:** Clean separation of page composition and feature components. |
| **Catalog & Cost Subsystem** | Full product catalog, SKU variants, retail prices, and baseline COGS management. | Dedicated feature in `src/features/catalog/` (`CatalogPage`, `CreateProductModal`, `EditVariantModal`, `useCatalog`). | **COMPLETE & ACTIVE:** SCR-04 operational with SKU activation, retail price, and COGS updates. |
| **Settlement Reconciliation** | Manual wallet payout entry with automated variance calculation and ledger status. | Manual wallet payout entry (`RecordSettlementModal`), automatic variance derivation, audit trail. | **COMPLETE & ACTIVE:** SCR-02 operational with real backend ledger and discrepancy logging. |
| **Discrepancy View Location** | Embedded audit tab and drawer in `SettlementsPage` (SCR-02). | Embedded audit drawer & tab inside `SettlementsPage`, plus dedicated resolve action modal. | **COMPLETE & ACTIVE:** Discrepancies viewed and resolved (`ResolveDiscrepancyModal`) within settlement workspace. |
| **Profit & Analytics Metrics** | 5 core financial KPIs: Gross Revenue, Fees, Projected Settlement, COGS, Contribution Profit + Margin %. | 5 core financial KPIs, dynamic trend visualizer, top SKUs, and channel breakdown. | **COMPLETE & ACTIVE:** SCR-03 operational with DeliveredAt recognition, trend visualizer, and order drilldown. |
| **Data Access & Services** | Co-located feature services (`ordersApi`, `settlementsApi`, `analyticsApi`, `catalogApi`, `discrepanciesApi`). | Centralized HTTP client (`httpClient`) with Axios interceptors and bearer auth support. | **COMPLETE & ACTIVE:** Zero inline mock arrays; 100% backend API connectivity. |
| **Feature Hooks** | Co-located custom React hooks (`useOrders`, `useSettlement`, `useAnalytics`, `useCatalog`). | Custom hooks encapsulating loading, error, and server state synchronization. | **COMPLETE & ACTIVE:** All views consume domain state via custom hooks. |
| **Fee Calculation** | Real-time backend fee preview via `/orders/preview-fee`. | Canonical backend fee preview engine calculating commission, payment, service, and fixed fees. | **COMPLETE & ACTIVE:** `CreateOrderModal` queries live fee preview on input change. |
| **UI Copy & Localization** | Vietnamese UI for local operators with standard English retail/financial acronyms (SKU, POS, COGS). | Clear bilingual policy documented in Section 2.4 of UI/UX specification. | **COMPLETE & ACTIVE:** Intuitive Vietnamese layout with unambiguous financial acronyms. |
| **Financial Color Standards** | Design tokens for positive `#10b981`, negative `#ef4444`, warning `#f59e0b`, neutral `#64748b`. | Consistent badge variants (`success`, `danger`, `warning`) and MoneyText formatting. | **COMPLETE & ACTIVE:** Enforced across all tables, rhythm strips, and modals. |
