...
- **`OrderService` (`src/features/orders/api/OrderService.ts`):** Executes HTTP calls for order queries, creation, and lifecycle status transitions (`POST /api/orders`, `POST /api/orders/{id}/ship`, etc.).
- **`FeeService` (`src/features/orders/api/FeeService.ts`):** Executes HTTP calls for real-time backend fee calculation previews and channel fee schedule configurations (`POST /api/fees/preview`, `GET /api/fees/schedules`).
- **`SettlementService` (`src/features/settlements/api/SettlementService.ts`):** Executes HTTP calls for settlement ledger queries (`GET /api/settlements`), recording actual wallet payouts (`POST /api/settlements/{id}/record`), and reconciling projected vs. actual settlement (`POST /api/settlements/{id}/reconcile`). Does not include statement import endpoints in target MVP scope.
- **`DiscrepancyService` (`src/features/discrepancies/api/DiscrepancyService.ts`):** Executes HTTP calls for discrepancy audit records (`GET /api/discrepancies`) and resolution notes (`POST /api/discrepancies/{id}/resolve`).
- **`AnalyticsService` (`src/features/analytics/api/AnalyticsService.ts`):** Executes HTTP calls for executive revenue and profit KPIs (`GET /api/analytics/kpis`), financial trends (`GET /api/analytics/trends`), channel share (`GET /api/analytics/channels`), top SKU profit leaderboard (`GET /api/analytics/top-skus`), and CSV data export.
- **`CatalogService` (`src/features/catalog/api/CatalogService.ts`):** Executes HTTP calls for product queries (`GET /api/catalog/products`), variant lookups (`GET /api/catalog/variants`), product/SKU creation/update (`POST /api/catalog/products`), and price/cost maintenance (`PUT /api/catalog/variants/{id}/pricing`).
