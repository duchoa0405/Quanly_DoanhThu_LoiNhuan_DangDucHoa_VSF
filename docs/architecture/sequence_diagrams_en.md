...
sequenceDiagram
    ...
    alt User has Sales & Ops Role
        ...
        Ctrl->>Ctrl: Map to UpdateVariantCommand(id, CostPrice, ActorIdentity)
...
> **Endpoint:** `POST /fee-schedules` (`createFeeSchedule`)  
> **Source Files:** `features/settlements/components/FeeScheduleModal.tsx`, `hooks/useFeeSchedule.ts`, `features/settlements/api/FeeScheduleService.ts` $\rightarrow$ `FeeSchedulesController.cs` $\rightarrow$ `FeeScheduleService.cs` $\rightarrow$ `FeeScheduleRepository.cs`, `UnitOfWork.cs`  
...
sequenceDiagram
    ...
    Owner->>UI: Configures new Shopee rate (example configured schedule: Comm 5%, Pay 4%, Service 2.5%, Cap 30,000 VND, EffectiveFrom)
    ...
    alt Unauthorized Role
        ...
        Ctrl->>Ctrl: Map to CreateFeeScheduleCommand(..., ActorIdentity)
        ...
        Note over Svc, Uow: ATOMIC VERSIONING TRANSACTION (IUnitOfWork)
        Svc->>Uow: ExecuteTransactionAsync()
        opt Existing Active Schedule Exists
            Uow->>DB: 1. UPDATE fee_schedules SET is_active = FALSE, effective_to = @effectiveFrom WHERE id = @existingId
        end
        Uow->>DB: 2. INSERT INTO fee_schedules (channel, payment_method, commission_rate, payment_fee_rate, service_fee_rate, service_fee_cap, fixed_fee_per_order, effective_from, is_active = TRUE) VALUES (...)
        Uow->>DB: COMMIT TRANSACTION
        DB-->>Uow: Committed Successfully
        Uow-->>Svc: Completed
        ...
        ApiClient-->>Hook: trigger data refresh
...
> **Endpoint:** `POST /orders/preview-fee` (`previewOrderFees`)  
...
sequenceDiagram
    ...
    Note over Hook: UI may debounce preview requests to minimize network roundtrips
    ...
    Engine->>Repo: GetActiveScheduleAsync(channel, paymentMethod, DateOnly.FromDateTime(DateTime.UtcNow))
    ...
    Engine->>Factory: GetStrategy(ChannelType.SHOPEE)
    Factory-->>Engine: ShopeeFeeStrategy instance

    Engine->>Strat: Calculate(subtotal: 1,000,000, voucher: 50,000, schedule)
    Note over Strat: In-memory evaluation against active schedule parameters:<br/>GrossRevenue = 1,000,000 - 50,000 = 950,000<br/>Commission = 1,000,000 * 4.5% = 45,000<br/>PaymentFee = 950,000 * 4% = 38,000<br/>ServiceFee = min(1,000,000 * 2.0%, schedule.ServiceFeeCap)<br/>FixedFee = schedule.FixedFeePerOrder<br/>TotalPlatformFees = sum(fees)<br/>ProjectedSettlement = GrossRevenue - TotalPlatformFees
...
> **Source Files:** `features/orders/components/CreateOrderModal.tsx`, `hooks/useOrders.ts`, `api/OrderService.ts` $\rightarrow$ `OrdersController.cs` $\rightarrow$ `OrderService.cs` $\rightarrow$ `ProductRepository.cs`, `OrderRepository.cs`, `UnitOfWork.cs`  
> **Tables:** `orders`, `order_items`, `order_status_history`, `product_variants`  
...
sequenceDiagram
    ...
    Staff->>UI: Submits Order (externalOrderId, channel, paymentMethod, customerName, voucher, variant items)
    ...
    Ctrl->>Ctrl: Extract ActorIdentity (string) from JWT Claims
    Ctrl->>Ctrl: Map to CreateOrderCommand(..., ActorIdentity)
    Ctrl->>Svc: CreateOrderAsync(command)

    Svc->>OrderRepo: GetByExternalIdAsync(command.Channel, command.ExternalOrderId)
    OrderRepo->>DB: SELECT id FROM orders WHERE channel = @channel AND external_order_id = @externalOrderId
    DB-->>OrderRepo: Match found (if duplicate)
    alt Duplicate External Order Detected
        OrderRepo-->>Svc: Existing Order
        Svc-->>Ctrl: DuplicateOrderException("External order already registered for this channel")
...
> **Source Files:** `features/orders/components/OrderStatusActions.tsx`, `hooks/useOrders.ts`, `api/OrderService.ts` $\rightarrow$ `OrdersController.cs` $\rightarrow$ `OrderService.cs` $\rightarrow$ `OrderRepository.cs`, `DynamicFeeEngine.cs`, `UnitOfWork.cs`  
...
sequenceDiagram
    ...
    UI->>ApiClient: updateOrderStatus(orderId, { toStatus: 'SHIPPED' })
    ApiClient->>Ctrl: PATCH /api/v1/orders/{id}/status (UpdateOrderStatusRequest)
    Ctrl->>Svc: UpdateOrderStatusAsync(UpdateOrderStatusCommand(orderId, SHIPPED, ActorIdentity))
    Svc->>Repo: UpdateAsync(orderWithShippedStatus)
    Repo->>DB: UPDATE orders SET status = 'SHIPPED' WHERE id = @id
    Svc->>Repo: AddStatusHistoryAsync(shippedHistory)
    Repo->>DB: INSERT INTO order_status_history (from_status = 'PENDING', to_status = 'SHIPPED', changed_by = @actorIdentity)
    ...
    UI->>ApiClient: updateOrderStatus(orderId, { toStatus: 'DELIVERED' })
    ApiClient->>Ctrl: PATCH /api/v1/orders/{id}/status (UpdateOrderStatusRequest)
    Ctrl->>Svc: UpdateOrderStatusAsync(UpdateOrderStatusCommand(orderId, DELIVERED, ActorIdentity))

    Svc->>Svc: Validate valid progression (SHIPPED -> DELIVERED)
    ...
    Uow->>DB: 2. INSERT INTO order_status_history (order_id, from_status = 'SHIPPED', to_status = 'DELIVERED', changed_by = @actorIdentity)
...
> **Endpoint:** `POST /orders/{id}/cancel` (`cancelOrder`)  
> **Source Files:** `features/orders/components/CancelOrderModal.tsx`, `hooks/useOrders.ts`, `api/OrderService.ts` $\rightarrow$ `OrdersController.cs` $\rightarrow$ `OrderService.cs` $\rightarrow$ `OrderRepository.cs`, `UnitOfWork.cs`  
...
sequenceDiagram
    ...
    Staff->>UI: Submits cancellation with reason: "Customer requested cancellation prior to shipment"
    UI->>Hook: cancelOrder(orderId, { cancellationReason: 'Customer requested cancellation' })
    Hook->>ApiClient: cancelOrder(orderId, payload)
    ApiClient->>Ctrl: POST /api/v1/orders/{id}/cancel (CancelOrderRequest)
    Ctrl->>Ctrl: Extract ActorIdentity from JWT
    Ctrl->>Ctrl: Map to CancelOrderCommand(orderId, CancellationReason, ActorIdentity)
    Ctrl->>Svc: CancelOrderAsync(command)
    ...
    alt Order status is DELIVERED
        ...
        Note over Svc, Uow: ATOMIC TRANSACTION BOUNDARY (IUnitOfWork)
        Svc->>Uow: ExecuteTransactionAsync()
        Uow->>DB: 1. UPDATE orders SET status = 'CANCELLED', updated_at = NOW() WHERE id = @id
        Uow->>DB: 2. INSERT INTO order_status_history (order_id, from_status = @prev, to_status = 'CANCELLED', change_reason = @reason, changed_by = @actorIdentity)
        Uow->>DB: COMMIT TRANSACTION
        DB-->>Uow: Committed
        Uow-->>Svc: Success
        ...
        Ctrl-->>ApiClient: HTTP 200 OK (OrderDetailResponse)
        ApiClient-->>Hook: Order cancelled
        Hook-->>UI: Update status badge to CANCELLED
...
> **Endpoint:** `GET /settlements` (`getSettlementLedger`), `GET /settlements/summary` (`getSettlementSummary`)  
> **Source Files:** `features/settlements/components/SettlementLedger.tsx`, `hooks/useSettlement.ts`, `api/SettlementService.ts` $\rightarrow$ `SettlementController.cs` $\rightarrow$ `SettlementService.cs` $\rightarrow$ `ReconciliationRepository.cs`  
> **Tables:** `reconciliation_records`, `orders`, `order_fee_snapshots`  
...
sequenceDiagram
    ...
    participant UI as SettlementLedger
    ...
    alt Unauthorized Role (Sales Staff)
        ...
        Repo->>DB: SELECT r.id, r.order_id, o.external_order_id, o.channel, o.gross_revenue, s.commission_fee_amount, s.payment_fee_amount, s.service_fee_amount, s.fixed_fee_amount, s.total_platform_fees, r.projected_settlement, r.actual_settlement, r.variance_amount, r.status, r.reconciled_at FROM reconciliation_records r JOIN orders o ON r.order_id = o.id JOIN order_fee_snapshots s ON o.id = s.order_id WHERE ... ORDER BY o.order_date DESC LIMIT 20
        DB-->>Repo: Paged rows
...
> **Source Files:** `features/settlements/components/RecordSettlementModal.tsx`, `hooks/useSettlement.ts`, `api/SettlementService.ts` $\rightarrow$ `SettlementController.cs` $\rightarrow$ `SettlementService.cs` $\rightarrow$ `ReconciliationRepository.cs`, `DiscrepancyRepository.cs`, `UnitOfWork.cs`  
...
sequenceDiagram
    ...
    UI->>Hook: reconcileOrder(orderId, { actualSettlement: 400000, notes, discrepancyType })
    ...
    Ctrl->>Ctrl: Extract ActorIdentity (string) from JWT
    Ctrl->>Ctrl: Map to ReconcileSettlementCommand(orderId, ActualSettlement, Notes, DiscrepancyType, ActorIdentity)
    ...
    alt Variance == 0 (Exact Match)
        Note over Svc: Perfect settlement: No fees disputed (notes & discrepancyType optional)
        Svc->>Uow: ExecuteTransactionAsync()
        Uow->>DB: UPDATE reconciliation_records SET actual_settlement = @act, variance_amount = 0, status = 'RECONCILED', reconciled_at = NOW(), reconciled_by = @actorIdentity WHERE id = @id
        Uow->>DB: COMMIT TRANSACTION
    ...
    else Variance != 0 AND (Notes Missing OR DiscrepancyType Missing)
        Note over Svc: Validation Guard: Discrepancies require both justification note and discrepancyType
        Svc-->>Ctrl: ValidationException("Notes and discrepancyType are required when variance != 0")
        Ctrl-->>ApiClient: HTTP 422 Unprocessable Entity ("Notes and discrepancyType required")
        ApiClient-->>UI: Highlight mandatory fields
        UI-->>Fin: Prompts: "Please select discrepancy type and provide explanation notes"

    else Variance != 0 AND Notes Provided AND DiscrepancyType Provided
        Note over Svc, Uow: Discrepancy Branch: ATOMIC TRANSACTION (IUnitOfWork)
        Svc->>Uow: ExecuteTransactionAsync()
        Uow->>DB: 1. UPDATE reconciliation_records SET actual_settlement = @act, variance_amount = @var, status = 'DISCREPANCY', reconciliation_notes = @notes, reconciled_at = NOW(), reconciled_by = @actorIdentity WHERE id = @id
        Uow->>DB: 2. INSERT INTO discrepancy_audits (reconciliation_record_id, discrepancy_type = @type, explanation_note = @notes, created_at = NOW()) VALUES (...)
        Uow->>DB: COMMIT TRANSACTION
        DB-->>Uow: Committed
...
sequenceDiagram
    ...
    Note over Ctrl: Authorization Guard: Requires FinanceManager or ShopOwner
    Ctrl->>Ctrl: Extract ActorIdentity (string) from JWT
    Ctrl->>Ctrl: Map to ResolveDiscrepancyCommand(id, ResolutionNotes, ActorIdentity)
    ...
    Svc->>Svc: Apply resolution:<br/>resolutionNotes = command.ResolutionNotes<br/>resolvedBy = command.ActorIdentity<br/>resolvedAt = DateTime.UtcNow
    Note over Svc: isResolved is derived dynamically: (resolvedAt != null)
    Svc->>Repo: UpdateAsync(audit)
    Repo->>DB: UPDATE discrepancy_audits SET resolution_notes = @notes, resolved_by = @actorIdentity, resolved_at = NOW() WHERE id = @id
    DB-->>Repo: 1 row affected
    Repo-->>Svc: Completed
    ...
    ApiClient-->>Hook: refresh discrepancy data
...
sequenceDiagram
    ...
    UI->>Hook: fetchKpis({ from, to, channel: 'ALL' })
    Hook->>ApiClient: getFinancialKpis(params)
    ApiClient->>Ctrl: GET /api/v1/analytics/kpis?from=...&to=...
    ...
    UI->>ApiClient: exportCsv({ from, to })
    ApiClient->>Ctrl: GET /api/v1/analytics/export-csv?from=...&to=...
    Ctrl->>Svc: ExportCsvAsync(filter)
    Svc->>Repo: QueryRawExportDataAsync(filter)
    Repo->>DB: SELECT o.external_order_id, o.order_date, o.channel, o.gross_revenue, s.total_platform_fees, s.projected_settlement, SUM(i.quantity * i.unit_cost_snapshot) AS cogs, (s.projected_settlement - SUM(i.quantity * i.unit_cost_snapshot)) AS contribution_profit FROM orders o JOIN order_fee_snapshots s ON o.id = s.order_id JOIN order_items i ON o.id = i.order_id WHERE o.status = 'DELIVERED' GROUP BY o.id, s.id ORDER BY o.order_date DESC
