...
| **Analytics API** | Presentation | `AnalyticsController.cs` | Exposes executive KPI cards (Gross Revenue, Fees, Net Revenue, COGS, Contribution Profit), profit trends, channel share, top SKU, and CSV export endpoints. | `IAnalyticsService` | **Partial** |
...
| **Settlement Service** | Application Service | `SettlementService.cs` *(Prototype: `StatementMatchingService.cs`)* | Coordinates manual wallet settlement entries, computes mathematical variance (`projected_settlement - actual_settlement`), and records `RECONCILED` or `DISCREPANCY` states. | `IReconciliationRepository` | **Partial** |
...
| **Analytics Service** | Application Service | `AnalyticsService.cs` | Computes real-time financial KPIs (Gross Revenue, Platform Fees, Net Realized Revenue, COGS, Contribution Profit, Contribution Margin %), financial trends, channel share, and SKU contribution profit rankings. | `IAnalyticsRepository` *(Target Query Port)* | **Prototype** |
...
- **Target Controllers:**
  ...
  - `AnalyticsController`: Exposes dynamic analytical endpoints for executive KPI cards (5 cards: Gross Revenue, Platform Fees, Net Realized Revenue, COGS, Contribution Profit), trend charts, channel share, and top SKUs.
...
| **Settlement Service** | Manual settlement reconciliation | `StatementMatchingService.cs` exists as prototype with mock statement import; target manual reconciliation flow pending. | **Prototype** |
